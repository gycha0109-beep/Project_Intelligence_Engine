...
def _build(*, credential_scope_proven: bool) -> dict:
    return build_controlled_nonprod_report(
        ...
        dispatch_updated_at="2026-08-22T05:41:00Z",
        rollback_updated_at="2026-08-22T05:42:00Z",
...
class ControlledNonProductionExecutionTests(unittest.TestCase):
    ...
    def test_unproven_credential_scope_is_a_hard_blocker_even_after_verified_effect_and_rollback(self) -> None:
    ...
    def test_target_head_tamper_is_rejected(self) -> None:
        report = _build(credential_scope_proven=False)
        tampered = deepcopy(report)
        tampered["dispatch"]["postcondition_readback"]["head_sha"] = "3" * 40
        errors = verify_controlled_nonprod_report_data(tampered)
        self.assertTrue(any("dispatch head_sha" in error for error in errors))

    def test_rollback_state_tamper_is_rejected(self) -> None:
        report = _build(credential_scope_proven=False)
        tampered = deepcopy(report)
        tampered["rollback"]["postcondition_readback"]["draft"] = False
        errors = verify_controlled_nonprod_report_data(tampered)
        self.assertTrue(any("rollback postcondition readback" in error for error in errors))
