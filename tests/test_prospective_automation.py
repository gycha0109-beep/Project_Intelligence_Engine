...
def _analysis(root: Path, *, live_head: str = HEAD, local_head: str = HEAD):
    ...
    candidate.write_text(json.dumps({"candidate_id": "candidate-1"}), encoding="utf-8")
    source = {
        "repository": {"name_with_owner": "demo/repo"},
        "pull_request": {"number": 7, "head_oid": live_head},
...
class ProspectiveAutomationTests(unittest.TestCase):
    ...
    def test_no_trust_request_stops_at_waiting_for_trust_input(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((Path(result["bundle"]) / "manifest.json").is_file())
    ...
    def test_explicit_request_can_prepare_packet_without_recording_human_authority(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            workspace.mkdir()
            packet_path = root / "packet-from-writer.json"
            ...
            with (
                ...
                # Materialization normally writes the Trust report. The fake preserves that contract.
