...
def _analysis(root: Path):
    ...
    candidate.write_text(
        json.dumps({"candidate_id": "candidate-1", "status": "BLOCKED_OPERATOR_INPUT_REQUIRED"}),
        encoding="utf-8",
    )
    source = {
    ...
    def test_materialized_policy_request_reuses_existing_auto2_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("review_system.prospective_automation.prepare_review_packet", return_value={"packet_id": "packet-1"}),
