...
TRUST_GAP_MANIFEST_CONTRACT_VERSION = "PIE_HISTORICAL_TRUST_GAP_DIAGNOSTIC_MANIFEST_V1"
...
def _write_outputs(
    ...
    *,
    ...
    manifest_body = {
        "contract_version": TRUST_GAP_MANIFEST_CONTRACT_VERSION,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Extract historical MISSING_TRUST_FIELDS diagnostics from legacy "
            "PIE compact interface artifacts without rerunning evaluation."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not observations:
            raise GitHubBackfillError(
                "no historical MISSING_TRUST_FIELDS observations matched the requested PIE revision"
            )
        ...
        diagnostic = build_trust_gap_diagnostic(observations)
        _write_outputs(
            Path(args.output),
            pie_revision=args.pie_revision,
